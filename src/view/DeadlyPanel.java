...
/**
 * graphical representation of the state of the game in deadly mode. 
 ...
 *
 */
public class DeadlyPanel extends Panel {

	private GameDeadly gameDeadly;

	private BufferedImage backgroundPlay = UtilityMethods.loadImg("res/background3.png");

	//back to menu endgame button
	private JButton backToMenu;

	private JLabel endGameMessage = new JLabel();
	 ...
	 */
	public DeadlyPanel(GameDeadly gameDeadly) {

		super(gameDeadly);
		this.gameDeadly = gameDeadly;

		//cards left in the draw pile
		cardsRemained.setText(gameDeadly.getDeck().size()+"");
		...
		if(gameDeadly.getDiscardedCard().getType() == Type.WILDCARD) {
	...
	public void initHandsBplayers () {
		...
		for(int i=0; i<7; i++) {
			...
			handCardBPlayer.add(mapCardToCardB.get(gameDeadly.getPlayer().getHand().get(i)));
		}
	...
	public void update(Observable o, Object arg) {
		...
		if(!gameDeadly.getPlayerAI_1().isInGame())
			remove(messi);
		if(!gameDeadly.getPlayerAI_2().isInGame())
			remove(leclerc);
		if(!gameDeadly.getPlayerAI_3().isInGame())
			remove(elon);

		cardsRemained.setText(gameDeadly.getDeck().size()+"");

		
		if(gameDeadly.getGameStatus().equals("YOU LOSE") || gameDeadly.getGameStatus().equals("FINISHED")) {

			if(gameDeadly.getGameStatus().equals("YOU LOSE")){
				endGameMessage.setText("YOU LOSE!");
				endGameMessage.setBounds(595, 300, 350, 50);

			}
			else{
				endGameMessage.setText(gameDeadly.getWinner()+" WON THE GAME!");
				endGameMessage.setBounds(497, 300, 350, 50);
			}

			endGameMessage.setFont(new Font("Tahoma", Font.PLAIN, 23));
			endGameMessage.setForeground(new Color(255,0,0));
			...
			if (gameDeadly.getCounter() != 0) {
				...
				drawCardsLabel.setText("+"+gameDeadly.getCounter());
				add(drawCardsLabel);
			}

			displayCards();

			if (gameDeadly.getDrawCardsBoolean() && gameDeadly.getCounter()==0)	{

				remove(drawCardsLabel);
				repaint();
				gameDeadly.setDrawCardsBoolean(false);
			}
			...
			if (gameDeadly.getTurnIndex() == 0) {
				...
				if(firstTimeAddingRect) {
					add(rect);
					if(gameDeadly.getPlayer().getHand().size()>15) {
						add(rect2);
					}
					firstTimeAddingRect = false;
				}

				while(gameDeadly.isColorAnsw()) {
		...
		switch(gameDeadly.getTurnIndex()) {
		...
		case 1 -> {
			...
			if (gameDeadly.getPlayerAI_1().isUnoDisplayable()) {
			...
			gameDeadly.getPlayerAI_1().setUnoDisplayable(false);
		}
		...
		case 2 -> {			
			...
			if (gameDeadly.getPlayerAI_2().isUnoDisplayable()) {
			...
			gameDeadly.getPlayerAI_2().setUnoDisplayable(false);
		}
		...
		case 3 -> {
			...
			if (gameDeadly.getPlayerAI_3().isUnoDisplayable()) {
			...
			gameDeadly.getPlayerAI_3().setUnoDisplayable(false);
		}
		};
	}
	...
	public void displayCardsP1() {
		...
		//only one row is needed
			...
			int width = (gameDeadly.getPlayer().getHand().size())*62;
			int startingPoint = 640-width/2;
	...
	public void displayCardsAI_1() {
		...
		List<Card> hand = gameDeadly.getPlayerAI_1().getHand(); 
	...
	public void displayCardsAI_2() {
		...
		List<Card> hand = gameDeadly.getPlayerAI_2().getHand();
		int width = hand.size()*21;
		int startingPoint = 640-width/2;
	...
	public void displayCardsAI_3() {
		...
		List<Card> hand = gameDeadly.getPlayerAI_3().getHand();
		int width = hand.size()*21;
		int startingPoint = 320-width/2;
